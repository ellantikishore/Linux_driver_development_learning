...
#define MY_PROC_ENTRY "kishore_new"
#define PROC_FULL_PATH "/proc/kishore_new"

extern unsigned long kishore_icmp_in_stats;
...
static ssize_t my_proc_write(struct file *filp, const char * buffer, size_t count, loff_t *pos)
{
//    get_user(kishore_icmp_in_stats, (unsigned long __user*)buffer);
...
//    return count;
    ...
    *pos = (int) count;
    len = count-1;
    kstrtol(data, 10, &kishore_icmp_in_stats);
...
ssize_t my_proc_read(struct file *filp,char *buf, size_t count, loff_t *offp )
{
    ...
    sprintf(DATA, "icmp packets received = %lu", kishore_icmp_in_stats);
...
int create_new_proc_entry(void) {
    ...
    //char DATA[DATA_SIZE];
    //sprintf(DATA, "icap packets received = %u", kishore_icmp_in_stats);
